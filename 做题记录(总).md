...
第一行包含两个用空格隔开的正整数𝑁和𝐷(3 ≤ 𝑁 ≤ 1000,1 ≤ 𝐷 ≤ 10^9)，含义如题面所示。

接下来的𝑁行，每行一个长度为𝑁的字符串，表示网格图的一行，每一个字符为’.’,’#’,’S’中的一个，’.’和’S’均为空地，但是’S’是可能的虫群女王位于的起始位置。’#’则是被安放了炸弹的位置，第一行，第一列，最后一行，最后一列的所有字符均为’#’。


### 输出描述

输出一个整数，表示可能在某一个时刻出现虫群的方格的数量。
...
输入的第一行包含一个正整数𝑄(1 ≤ 𝑄 ≤ 10^4)表示询问的数量。

接下来的𝑄行，每行包含三个整数𝑑𝑖, 𝑥𝑖和𝑦𝑖(0 ≤ 𝑥𝑖, 𝑦𝑖, 𝑑𝑖 ≤ 10^18)表示一个询问。

### 输出描述

输出𝑄行，对于每一个询问输出一行一个整数表示询问的答案。
...
第一行输入一个正整数𝑁(2 ≤ 𝑁 ≤ 10^5)，含义见题面。

接下来的𝑁行，每五个用空格分开的整数𝑐𝑖, 𝑝{𝑖,1}, 𝑝{𝑖,2}, 𝑝{𝑖,3}, 𝑝{𝑖,4}(1 ≤ 𝑐𝑖 ≤ 103, 1 ≤ 𝑝{𝑖,𝑗} ≤ 2𝑁)，分别表示重新排列该魔法阵节点需要的代价以及该魔法阵内四个回路节点的编号，输入数据保证所有1~2𝑁的回路节点编号会出现且仅会出现两次。

### 输出描述

输出一行一个正整数，表示最小的使得所有回路节点互相连通的代价。
...
van now[MaxN],to[MaxN]; van n,k,m;
van a[MaxN],b[MaxN],fa[MaxN],siz[MaxN];
vector<van> elem[MaxN]; van ans[MaxN];
van GetFa(van x) {return fa[x]==x?x:fa[x]=GetFa(fa[x]);}
void Merge(van a,van b) {fa[GetFa(a)]=GetFa(b);}
set<van> goes[MaxN];
int main() {
//	freopen("assassinate.in","r",stdin);
//	freopen("assassinate.out","w",stdout);
	read(n),read(k),read(m); for (int i=1;i<=n;i++) now[i]=fa[i]=i;
	for (int i=1;i<=k;i++) {
		read(a[i]),read(b[i]);
		swap(now[a[i]],now[b[i]]);
	} for (int i=1;i<=n;i++) to[now[i]]=i;
	for (int i=1;i<=n;i++) Merge(i,to[i]);
	for (int i=1;i<=n;i++) siz[GetFa(i)]++,elem[GetFa(i)].push_back(i);
	for (int i=1;i<=n;i++) now[i]=i;
	for (int i=1;i<=n;i++) {
		if (GetFa(i)!=i) continue;
		if (siz[i]*k<=m) for (int j=0;j<elem[i].size();j++) ans[elem[i][j]]=siz[i];
		else {
			for (int j=0;j<elem[i].size();j++) goes[elem[i][j]].insert(elem[i][j]);
			for (int j=1;j<=m;j++) {
				van kk=(j-1)%k+1;
				swap(now[a[kk]],now[b[kk]]);
				goes[now[a[kk]]].insert(a[kk]);
				goes[now[b[kk]]].insert(b[kk]);
			} for (int j=0;j<elem[i].size();j++)
				ans[elem[i][j]]=goes[elem[i][j]].size();
		}
