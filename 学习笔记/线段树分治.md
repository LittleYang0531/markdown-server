...
const van MaxN=1e6+10,MaxM=1e6+10;
struct query {
	van a,b,w,id; query(){};
...
void solve(van L,van R,van qL,van qR,van d=0) { 
	...
	// 注意当结果为L时，所有为L都可以被加入，所以这里是小于等于而不是小于 
	...
	else ans[mid]=-1; // 这里如果不满足可以直接判无解，不需要新建元素 
	while (times) S.restore(),times--; // 还原之前的计算操作
	// 加入[1,qL]里权值小于等于ans[mid]的边
